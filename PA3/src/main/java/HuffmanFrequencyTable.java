 ...
 *
 * This class creates and maintains a table. For example for a string
 * "AAAABBCCC" ======================================== char frequency Code
 * ---------------------------------------- A 4 1 B 2 11 C 3 10
 * ========================================
 *
...
public class HuffmanFrequencyTable {
	...
	public LinkedList<TableItem> getTable() {
		return table;
	}
	...
	public char getChar(String code) {
		for (int i = 0; i < table.size(); i++) {
			if (table.get(i).getCode().equals(code)) {
	...
	public static void main(String args[]) {
		...
		for (int i = 0; i < hf.table.size(); i++) {
			System.out.println(
					hf.table.get(i).getInputChar()
					+ "  " + hf.table.get(i).getFreq() + "  "
					+ hf.getCode(hf.table.get(i).getInputChar()));
		}
	}
}